...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Smoothly update the background image
    function updateBackgroundImage(index) {
        const imageUrl = images[index].src;
        if (imageUrl) {
            sliderBackground.style.transition = 'background-image 1s ease'; // Smooth transition for background
            sliderBackground.style.backgroundImage = `url(${imageUrl})`;
        }
    }

    // Smooth transition for the slide animation
    function goToSlide(index) {
        const slideWidth = slides[0].offsetWidth + 20; // Account for 20px gap
        slider.style.transition = 'transform 1s ease'; // Smooth transition
        slider.style.transform = `translateX(calc(50% - ${slideWidth / 2}px - ${index * slideWidth}px))`;

        navButtons.forEach((btn) => btn.classList.remove('active'));
        navButtons[index].classList.add('active');

        updateBackgroundImage(index); // Update background smoothly
        currentIndex = index;
    }
    ...
    updateBackgroundImage(0); // Set the initial background
});
